...
class FakesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        cycles_img = self.transform(Image.open(cycles_path).convert("RGB"))
...
# # Visualize the image we are working with
# cur_img = X[0][-3:]
# print(X[0][-3:].size())
...
for i in range(len(X)):
    saliency = compute_saliency_map(X[i:i+1], y, model)
    saliency = saliency.numpy()
    print(saliency.max())
    print(saliency.min())
    image = (saliency - saliency.min()) / (saliency.max()-saliency.min()) * 255.0
    print(type(image[0][0][0]))
    saliency_i = Image.fromarray(image)
    saliency_i = saliency_i.convert('L')
    saliency_i.save('Saliency_old/output_' + str(i) + '.png')

    cur_img = X[0][-3:]
    real_image = transform(cur_img)
    real_image = real_image.convert('RGB')
    real_image.save('Saliency_old/real_' + str(i) + '.png')
